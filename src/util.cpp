...
const char * const BITCOIN_CONF_FILENAME = "flash.conf";
const char * const BITCOIN_PID_FILENAME = "flashd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "flash" is a composite category enabling all Flash-related debug output
            if(ptrCategory->count(string("flash"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\FlashCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\FlashCore
    // Mac: ~/Library/Application Support/FlashCore
    // Unix: ~/.flashcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "FlashCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/FlashCore";
#else
    // Unix
    return pathRet / ".flashcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty flash.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override flash.conf

...
*Setup instructions for a Gitian build of Flash Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Flash
...
- [Building Flash Core](#building-flash-core)
...
Clone the git repositories for Flash Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/pmo24/flash
```
...
in the Flash Core repository under 'Fetch and build inputs' to install sources which require
...
To build Flash Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Flash Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/flash/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/pmo24/flash
...
For example:
```bash
URL=https://github.com/crowning-/flash.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit flash=${COMMIT} --url flash=${URL} ../flash/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit flash=${COMMIT} --url flash=${URL} ../flash/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit flash=${COMMIT} --url flash=${URL} ../flash/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the flash git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../flash/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/flash.git
SIGPATH=/some/root/path/flash-detached-sigs.git

./bin/gbuild --url flash=${BTCPATH},signature=${SIGPATH} ../flash/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/flash-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/flash-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/flash-osx-build.assert
```
...
[flash/gitian.sigs](https://github.com/pmo24/gitian.sigs/) repository, or if that's not possible to create a pull
request.
